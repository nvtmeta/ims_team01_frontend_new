...
const SelectCandidate = ({ recruiter, setRecruiter }: { recruiter: SelectInterface | undefined, setRecruiter: any }) => {
    ...
    const fetchMoreData = () => {
        if (recruiterList?.length < totalElements) {
            setTimeout(async () => {
                //todo:we can allow all candidate with status ...
                // todo: format date for general
        ...
        <div>
            <div className='flex flex-col  mt-4'>
                <div className='flex justify-start items-center '>
                    ...
                    <Popover
                        isOpen={isOpen} onOpenChange={(open) => setIsOpen(open)} size='lg' placement='right'>
                        <PopoverTrigger>
                            <Button
