...
const SelectManager = ({ recruiter, setRecruiter }: { recruiter: SelectInterface | undefined, setRecruiter: any }) => {
    ...
    const fetchMoreData = () => {
        if (recruiterList?.length < totalElements) {
            setTimeout(async () => {
                const dataNext = await fetchRecruiterListApi(`user/role/manager?page=${currentPageRecruiter + 1}&size=${SIZE}`)
    ...
    const loadInitRecruiterList = async () => {
        if (!isLoaded) { // Check if the function has already been called
            await fetchRecruiterListApi(`user/role/manager?page=0&size=${SIZE}`)
                .then(res => {
        ...
        <div>
            <div className='flex flex-col  mt-4'>
                <div className='flex justify-start items-center '>
                    <label className='w-40 font-bold' htmlFor='phone'>Approver{" "}
                    ...
                    <Popover isOpen={isOpen} onOpenChange={(open) => setIsOpen(open)} size='lg' placement='bottom'>
                        <PopoverTrigger >
                            <Button
                                ...
                                {recruiter ? recruiter.label : "Select an approver"}
                            </Button>
                        </PopoverTrigger>
                        <PopoverContent>
