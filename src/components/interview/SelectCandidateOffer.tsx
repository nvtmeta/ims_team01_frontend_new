...
const SelectCandidateOffer = ({ recruiter, setRecruiter }: { recruiter: SelectInterface | undefined, setRecruiter: any }) => {
    ...
    const fetchMoreData = () => {
        if (recruiterList?.length < totalElements) {
            setTimeout(async () => {
                const dataNext = await fetchRecruiterListApi(`candidate/status/no-ban?page=${currentPageRecruiter + 1}&size=${SIZE}`)
    ...
    const loadInitRecruiterList = async () => {
        if (!isLoaded) { // Check if the function has already been called
            await fetchRecruiterListApi(`candidate/status/no-ban?page=0&size=${SIZE}`)
                .then(res => {
                        ...
                        <PopoverContent>
                            <div id="scroll_infinite" className='max-h-96 w-96 overflow-y-auto '>
                                <InfiniteScroll
                                    ...
                                    next={fetchMoreData}
                                    hasMore={hasMore}
                                    loader={<Spinner className={`mx-auto flex justify-center ${recruiterList?.length == totalElements ? "hidden" : ""}`} label='Loading more candidates...' />}
...
export default SelectCandidateOffer